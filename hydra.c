...
void hydra_restore_write(int32_t print_msg) {
  ...
  for (j = 0; j < hydra_options.max_use; j++) {
    ...
    if (j == 0 && debug) {
      printf("[DEBUG] sizeof hydra_head: %llu\n", sizeof(hydra_head));
...
//    hydra_restore_read();
  ...
  printf("[DATA] max %d task%s per %d server%s, overall %d task%s, %llu login tr",
...
//    printf("[DATA] with additional data %s\n", hydra_options.miscptr);
    ...
    if (time(NULL) - elapsed_status > status_print) {
      ...
      for (j = 0; j < hydra_options.max_use; j++)
        if (hydra_heads[j]->active >= HEAD_UNUSED)
          k++;
      printf("[STATUS] %.2f tries/min, %llu tries in %02lu:%02luh, %llu to do in %02lu:%02luh, %d active\n", (1.0 * hydra_brains.sent) / (((elapsed_status - starttime) * 1.0) / 60),     // tries/min
  ...
  printf("%d of %d target%s%scompleted, %llu valid password", 
